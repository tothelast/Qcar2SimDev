...
Manages all scene actors in the QLabs Cityscape Lite environment:
- Autonomous QCar2 vehicles (circular route, roundabout route)
- Pedestrians crossing roads
- Parked vehicles
- Stop signs

This module encapsulates all scene setup and control logic.
"""
...
class SceneManager:
    ...
    Responsibilities:
    - Spawn and control autonomous QCar2 vehicles
    - Spawn and control pedestrians
    - Spawn static actors (parked vehicles, stop signs)
    - Manage all actor threads
    """
    ...
    def __init__(self, qlabs):
        ...
        """
        self.qlabs = qlabs
        self.circular_qcar = None
        self.roundabout_qcar = None
        self.pedestrians = []
        self.parked_vehicles = []
        self.stop_signs = []
        self.threads = []
        
    def spawn_circular_qcar(self):
        """
        Spawn QCar2 for circular route (Node 0 → 2 → 4 → 6 → 0).

        Returns:
            QLabsQCar2 instance or None if failed
        """
        print("\n" + "-"*70)
        print("Spawning Circular Route QCar2...")
        print("-"*70)

        self.circular_qcar = QLabsQCar2(self.qlabs)
        spawn_location = [0.000, 1.302, 0.005]  # Node 0
        spawn_rotation = [0.0, 0.0, -90.0]  # -90 degrees (facing south)
        
        status = self.circular_qcar.spawn_id_degrees(
            actorNumber=1,
        ...
        if status == 0:
            print(f"✓ Circular QCar2 spawned at Node 0 [{spawn_location[0]:.3f}, {spawn_location[1]:.3f}]")
            print(f"  Route: 0 → 2 → 4 → 6 → 0 (infinite loop)")
            print(f"  Movement will start after pedestrians are spawned")
            return self.circular_qcar
        else:
            print(f"✗ Failed to spawn circular QCar2. Status code: {status}")
            self.circular_qcar = None
            return None
    
    def spawn_roundabout_qcar(self):
        """
        Spawn QCar2 for roundabout route (Node 16 → 17 → 16).
        ...
        self.roundabout_qcar = QLabsQCar2(self.qlabs)
        spawn_location = [9.076, 37.098, 0.005]  # Node 16
        spawn_rotation = [0.0, 0.0, -80.6]  # -80.6 degrees
        
        status = self.roundabout_qcar.spawn_id_degrees(
            actorNumber=2,
            location=spawn_location,
            rotation=spawn_rotation,
            scale=[1.0, 1.0, 1.0],
        ...
        if status == 0:
            print(f"✓ Roundabout QCar2 spawned at Node 16 [{spawn_location[0]:.3f}, {spawn_location[1]:.3f}]")
            print(f"  Route: 16 → 17 → 16 (infinite loop)")
            print(f"  Movement will start after pedestrians are spawned")
            return self.roundabout_qcar
        else:
            print(f"✗ Failed to spawn roundabout QCar2. Status code: {status}")
            self.roundabout_qcar = None
            return None
    
    def spawn_parked_vehicles(self):
        """
        Spawn parked vehicles at strategic locations.
        
        Returns:
            List of QLabsQCar2 instances
        """
        print("\n" + "-"*70)
        ...
        for i, (location, rotation) in enumerate(parking_spots):
            try:
                ...
                if status == 0:
                    self.parked_vehicles.append(car)
                    print(f"  ✓ Parked car {i+1}/{len(parking_spots)} at: [{location[0]:.1f}, {location[1]:.1f}], rotation: {rotation}°")
                else:
                    print(f"  ✗ Failed to spawn parked car {i+1}/{len(parking_spots)}. Status: {status}")
                    
            except Exception as e:
                print(f"  ✗ Error spawning parked car {i+1}/{len(parking_spots)}: {e}")
        
        print("-"*70)
        print(f"Total parked vehicles: {len(self.parked_vehicles)}/{len(parking_spots)}")
        
        return self.parked_vehicles
    
    def spawn_stop_signs(self):
        """
        Spawn stop signs at strategic locations.
        
        Returns:
            List of QLabsStopSign instances
        """
        print("\n" + "-"*70)
        ...
        for i, (location, rotation) in enumerate(stop_sign_locations):
            try:
                stop_sign = QLabsStopSign(self.qlabs)
                ...
                if status == 0:
                    self.stop_signs.append(stop_sign)
                    print(f"  ✓ Stop sign {i+1}/{len(stop_sign_locations)} at: [{location[0]:.1f}, {location[1]:.1f}], rotation: {rotation}°")
                else:
                    print(f"  ✗ Failed to spawn stop sign {i+1}/{len(stop_sign_locations)}. Status: {status}")
                    
            except Exception as e:
                print(f"  ✗ Error spawning stop sign {i+1}/{len(stop_sign_locations)}: {e}")
        
        print("-"*70)
        print(f"Total stop signs: {len(self.stop_signs)}/{len(stop_sign_locations)}")
        
        return self.stop_signs
    
    def spawn_all_pedestrians(self):
        """
        Spawn all pedestrians at strategic crossing locations.

        Returns:
            List of QLabsPerson instances
        """
        print("\n" + "-"*70)
        print("Spawning Pedestrians Across the Map")
        print("-"*70)

        # Pedestrian crossing locations (EXACT positions from original implementation)
        # Format: (actor_number, curb_1, curb_2, crossing_direction, description)
        pedestrian_crossings = [
            ...
            (300, [-2.500, 18.498, 1.0], [5.186, 18.426, 1.0], 'vertical',
             "Near spawn (Edges 12→7 ↔ 6→13) - Crossing both parallel roads"),
            ...
            # Total crossing: ~7.7m (sidewalk + road + yellow divider + road + sidewalk)
            # crossing_direction='vertical' because the ROADS run North-South
            (301, [-21.891, 14.043, 1.0], [-14.508, 16.190, 1.0], 'vertical',
             "West section (Edges 22→9 ↔ 8→23) - Crossing both parallel roads"),
            ...
            (302, [-0.019, 39.767, 1.0], [-0.019, 47.474, 1.0], 'horizontal',
             "North section (Edges 23→21 ↔ 20→22) - Crossing both parallel roads"),
            ...
            # crossing_direction='vertical' because the ROADS run North-South
            (303, [16.921, 15.008, 1.0], [24.777, 15.008, 1.0], 'vertical',
             "East section (Edges 15→6 ↔ 7→14) - Crossing both parallel roads"),
        ]
        
        self.pedestrians = []
        
        for actor_number, curb_1, curb_2, crossing_direction, description in pedestrian_crossings:
            pedestrian = self._spawn_pedestrian_generic(
                actor_number, curb_1, curb_2, crossing_direction, description
            )
            if pedestrian:
                self.pedestrians.append(pedestrian)
        
        print("-"*70)
        print(f"Total pedestrians spawned: {len(self.pedestrians)}/{len(pedestrian_crossings)}")

        return self.pedestrians

    def _spawn_pedestrian_generic(self, actor_number, curb_1, curb_2, crossing_direction, description):
        """
        ...
        pedestrian = QLabsPerson(self.qlabs)
        ...
        if status == 0:
            ...
            movement_thread = threading.Thread(
                target=self._pedestrian_movement_loop,
    ...
    def _pedestrian_movement_loop(self, pedestrian, curb_1, curb_2, wait_offset_1, wait_offset_2):
        """
    ...
    def start_circular_qcar(self):
        """
        Start the circular QCar2 movement in a separate thread.

        Returns:
            Thread instance or None if circular_qcar not spawned
        """
        if not self.circular_qcar:
            print("✗ Cannot start circular QCar2 - not spawned")
            return None
        ...
        circular_thread = threading.Thread(
            target=self._circular_qcar_loop,
            args=(self.circular_qcar, route_nodes),
            daemon=True
        )
        circular_thread.start()
        self.threads.append(circular_thread)

        print(f"✓ Circular route thread started")
        print("-"*70)

        return circular_thread

    def start_roundabout_qcar(self):
        """
        Start the roundabout QCar2 movement in a separate thread.

        Returns:
            Thread instance or None if roundabout_qcar not spawned
        """
        if not self.roundabout_qcar:
            print("✗ Cannot start roundabout QCar2 - not spawned")
            return None
        ...
        roundabout_thread = threading.Thread(
            target=self._circular_qcar_loop,
            args=(self.roundabout_qcar, route_nodes),
            daemon=True
        )
        roundabout_thread.start()
        self.threads.append(roundabout_thread)

        print(f"✓ Roundabout route thread started")
        print("-"*70)

        return roundabout_thread

    def _circular_qcar_loop(self, qcar, route_nodes):
        """
        ...
        # Control parameters
        target_speed = 2.5  # m/s (full-scale speed)
