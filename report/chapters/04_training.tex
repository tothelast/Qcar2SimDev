...
Each recorded sample is a pair of one RGB JPEG image and one gzip-compressed JSON measurement dictionary. The measurement fields are selected to match what the SimLingo training pipeline expects and include: (i) a homogeneous ego pose matrix, (ii) a local route segment in the ego frame as well as the original route representation, (iii) two target points (primary and ``next'') used for target-point conditioning, and (iv) a scalar speed estimate. To support command-conditioning modes without requiring manual annotation, the current implementation assigns a constant high-level command label (\texttt{command=4}, ``follow the road'') in the per-frame measurement dictionary. All fields are computed online at each timestep during collection---no retroactive post-processing pass is required.
...
The root cause is a platform-level limitation: the QLabs API serializes all control and state-query interactions, so the aggregate command rate grows with the number of actors. The teleoperation controller issues a \texttt{set\_velocity\_and\_request\_state(...)} call at \SI{30}{Hz} (\path{data_collection/teleop_controller.py}, around lines 184--212), and each background actor thread requires its own API calls at every update step. To work around this serialization constraint, our implementation introduces three mitigations in \path{core/scene_spawner.py}: (i)~a shared mutex (\texttt{qlabs\_lock}, line~41) that prevents concurrent API calls from corrupting the communication channel, (ii)~a reduced default actor control update rate of \SI{2}{Hz} (lines~366--367) to limit lock contention with the main control thread, and (iii)~staggered actor thread start times with a 0--99~ms offset per actor (lines~462--464) to spread initial API bursts.
...
The final collected dataset comprises \textbf{40} training runs (7\,498~frames) and \textbf{15} validation runs (2\,997~frames), totalling 10\,495~frames across 55~runs. Individual run lengths range from 98 to 301~frames for training and 171 to 234~frames for validation, reflecting variation in route-completion time under teleoperation. Each frame consists of one RGB JPEG image and one gzip-compressed JSON measurement file; the combined on-disk footprint is approximately \SI{470}{\mebi\byte}.

Although 7\,498 training frames is modest by general deep-learning standards, the effective number of trainable parameters is small: LoRA (rank~32, \texttt{target\_modules="all-linear"}) applied to the Qwen2-0.5B language model introduces approximately 17.6\,M adapter parameters, while the 300\,M-parameter vision encoder remains frozen. Combined with the narrow task scope (a single route type with static-obstacle variations) and the fact that the pretrained CARLA checkpoint already encodes general driving behaviour, the dataset provides sufficient coverage for the domain-transfer fine-tuning objective pursued here. The main limitation is therefore not sample count per se, but scenario diversity---a point discussed further in \Cref{ch:limitations}.

\section{SimLingo-compatible dataset writer}
\label{sec:data-recorder}
The \texttt{DataRecorder} class in \path{data_collection/data_recorder.py} creates the run directory, writes RGB images and measurement JSONs, and produces a \texttt{results.json.gz} file so that existing SimLingo dataloaders can treat the QLabs run like a CARLA route.
