...
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners, minimum height=1cm, minimum width=2.2cm, align=center, font=\small},
  arrow/.style={-{Stealth[length=2.5mm]}, thick},
  label/.style={font=\scriptsize, midway},
]
  % Upper level
  \node[block, fill=blue!10] (vision) at (0,0) {Vision\\Encoder};
  \node[block, fill=blue!10] (llm) at (3.5,0) {Language\\Model};
  \node[block, fill=orange!10] (lateral) at (7.5,0.5) {Lateral PID};
  \node[block, fill=orange!10] (speed) at (7.5,-0.8) {Speed\\Controller};
  \node[block, fill=green!10] (vehicle) at (11,0) {Vehicle};

  % Inputs
  \node[font=\small, left=0.6cm of vision] (img) {Camera};
  \node[font=\small, below=0.6cm of llm] (nav) {Speed, Target Point};

  % Arrows
  \draw[arrow] (img) -- (vision);
  \draw[arrow] (vision) -- node[label, above] {visual tokens} (llm);
  \draw[arrow] (nav) -- (llm);
  \draw[arrow] (llm.east) ++(0,0.2) -- node[label, above] {route WPs} (lateral.west);
  \draw[arrow] (llm.east) ++(0,-0.2) -- node[label, below] {speed WPs} (speed.west);
  \draw[arrow] (lateral.east) -- node[label, above] {steer} (vehicle.west |- lateral.east);
  \draw[arrow] (speed.east) -- node[label, below] {throttle} (vehicle.west |- speed.east);

  % Level labels
  \draw[dashed, gray] (5.5,-1.6) -- (5.5,1.4);
  \node[font=\scriptsize\itshape, gray] at (1.75,1.5) {Upper level (learned)};
  \node[font=\scriptsize\itshape, gray] at (9.25,1.5) {Lower level (classical)};
\end{tikzpicture}
\caption{Two-level driving architecture. The learned upper level predicts future waypoints from vision and route intent; the classical lower level converts waypoints to steering and throttle via PID controllers.}
\label{fig:two-level}
\end{figure}
...
\medskip
\noindent\texttt{\small <system> Current speed: 2.5\,m/s. <img>[512 visual tokens]</img> Target waypoint: [nav$_1$][nav$_2$]. Predict the waypoints. <assistant> Waypoints: $|$ $\mathbf{q}_{\text{route}} \times 20$ $|$ $\mathbf{q}_{\text{speed}} \times 10$}
\medskip
