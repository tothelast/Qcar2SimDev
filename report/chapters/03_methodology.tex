...
QLabs scenarios specify an ego route as a sequence of world-frame waypoints (Appendix \Cref{app:scene-configs}). At runtime, the route manager implemented in \path{inference/route_manager.py} selects a lookahead target point along the polyline and converts it into the ego frame for prompt conditioning.
