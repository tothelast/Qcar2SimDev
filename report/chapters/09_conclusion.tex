...
\begin{itemize}[leftmargin=*,itemsep=0.25em]
  ...
  \item \textbf{Training configuration:} LoRA-based fine-tuning of an InternVL2-1B backbone using Hydra, PyTorch Lightning, and DeepSpeed, with hyperparameters recorded in Appendix \Cref{app:hyperparameters}.
...
Final quantitative results are left as structured placeholders (\Cref{ch:results}), but the codebase provides the instrumentation needed to run repeatable sweeps and to populate those tables. The remaining experimental gaps---including compute environment documentation, calibration validation, and comprehensive evaluation runs---are tracked in Appendix \Cref{app:open-questions}.
