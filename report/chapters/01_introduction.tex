...
\begin{enumerate}[leftmargin=*,itemsep=0.25em]
  ...
  \item \textbf{Parameter-efficient domain transfer:} How can a CARLA-pretrained VLA model be fine-tuned to QLabs data using LoRA adapters while preserving the knowledge learned from CARLA?
  \item \textbf{Runtime alignment:} How should control-loop timing, PID conversion, steering conventions, and ego-frame target-point computation be adapted so that the inference-time signal distribution matches the training distribution?
