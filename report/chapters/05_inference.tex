...
SimLingo-style conditioning encodes both the current speed and navigational intent into the prompt (see \Cref{sec:prompt-construction} for the general template). Although the SimLingo architecture supports both target-point and command conditioning modes, \textbf{this work uses target-point mode exclusively} because no language-annotated route data was collected for QLabs (see \Cref{sec:final-dataset-scope} for the rationale). The inference code in \path{inference/simlingo_model.py} retains support for both modes, but command mode was neither fine-tuned nor evaluated.
