...
\paragraph{{Training speed profile and obstacle response.}}
{To understand why the model's obstacle response varies across variants, we compare the expert training speed profile with the model's inference behaviour. The 55~training demonstrations comprise 20~obstacle-free runs (full 89~m circuit) and 35~obstacle runs in which the expert stopped before one of five obstacle placements (8~runs at V1, 6 at V2, 7 at V3, 9 at V4, and 5 at V5).} \Cref{fig:training-speed-profile} {shows the expert's average speed along the route, computed from the obstacle-free runs only. The profile increases from zero at the start to approximately 2.7~m/s by the roundabout exit (50--55~m), then fluctuates between 2.2 and 2.6~m/s through the straight section and final curve. The five obstacle locations are overlaid at their respective route progress positions: V1 at 26.9~m, V2 at 39.0~m, V3 at 51.8~m, V4 at 68.5~m, and V5 at 78.3~m.}

{The model's obstacle-free inference speed (blue dashed in} \Cref{fig:training-speed-profile}{) tracks the expert profile in the first half of the route but progressively exceeds it from approximately 40~m onward, reaching 3.5~m/s on the straight section (V4) compared to the expert's 2.4~m/s. This speed overshoot is consistent with the model having learned a policy that generalises the expert's acceleration patterns but overshoots the expert's cruising speed at later route positions where fewer training examples are available.}

{The obstacle response outcomes are best explained by the interaction of three factors:}

\begin{enumerate}
  \item {\emph{Approach speed.} V1 and V3 benefit from lower approach speeds (2.2 and 3.1~m/s in obstacle-free inference), providing more time and distance to decelerate once the obstacle is detected. V4 and V5 have approach speeds of 3.5 and 3.4~m/s respectively, leaving less margin.}

  \item {\emph{Visual lead time.} Road geometry determines how early the obstacle enters the camera's field of view. On straight segments (V1, V3, V4), the obstacle is visible 10--15~steps before arrival. On the tight curve at V5, the obstacle is occluded until the vehicle is within a few metres, providing almost no reaction time. This explains why V3 succeeds at a higher speed than V2: the straight exit segment provides earlier visibility, whereas V2 is on a mid-roundabout curve with moderate occlusion.}

  \item {\emph{Training data composition.} The number of obstacle-avoidance demonstrations varies by position: V4 has 9~expert obstacle runs while V5 has only 5. Additionally, the number of obstacle-free runs that drive through each position decreases along the route (42~at V1, 20~at V5), reducing the overall data density at later positions and potentially limiting the model's ability to generalise obstacle detection in those regions.}
\end{enumerate}

\Cref{fig:approach-speed-comparison} {compares the model's inference speed during obstacle approach against both the obstacle-free training speed envelope and the individual expert obstacle-avoidance demonstrations for each variant. In V1--V4, the model (blue solid) decelerates below the obstacle-free training envelope (gray band), following a pattern qualitatively similar to the expert obstacle runs (green lines)---confirming that obstacle detection is occurring---but only V1 and V3 decelerate early enough to stop without contact. In V5, the inference speed tracks the obstacle-free baseline (dark blue dashed) without any deviation, consistent with the model failing to perceive the obstacle entirely. These results suggest that the model's obstacle avoidance capability is not a single learned skill but an emergent interaction between its route-dependent speed prior, the geometry-dependent visual detection window, and the composition of training examples at each route position.}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{training_speed_profile.png}
  \caption{{Expert speed profile along the route from obstacle-free training runs. Gray band: mean $\pm$~1~std; dark gray line: mean. Blue dashed: model's obstacle-free inference speed (average of 4~baseline runs). Vertical lines mark obstacle locations, colour-coded by test outcome: green (clean stop, V1/V3), orange (low-speed contact, V2/V4), red (collision, V5). Annotations show the number of obstacle-free training routes that pass through each position ($n$). The model's inference speed exceeds the expert profile from approximately 40~m onward.}}
  \label{fig:training-speed-profile}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{approach_speed_comparison.png}
  \caption{{Per-variant speed comparison during obstacle approach. Gray band: expert obstacle-free speed envelope (mean $\pm$~1~std). Green lines: individual expert obstacle-avoidance demonstrations from training. Dark blue dashed: model's obstacle-free inference speed. Blue solid: model's inference speed with obstacle present (one representative run per variant). Red dashed line: obstacle location; red circles: vehicle--obstacle contact. In V1--V4, the model decelerates below the obstacle-free envelope, following a similar pattern to the expert obstacle runs. In V5, the model's speed tracks the obstacle-free baseline without deviation, consistent with failure to detect the obstacle.}}
  \label{fig:approach-speed-comparison}
\end{figure}
