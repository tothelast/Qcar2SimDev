...
This chapter presents the experimental evaluation of the fine-tuned SimLingo model on the QLabs roundabout scenario and compares it against a LiDAR-based adaptive cruise control (ACC) baseline. Both controllers are evaluated on the same test matrix using identical metrics (described in \Cref{ch:evaluation}).
...
\Cref{fig:trajectory-overlays} shows bird's-eye trajectory overlays for each obstacle variant. On the roundabout curve (top-right arc of each subplot), the ACC trajectories (orange dashed) are visibly offset inward from the route centerline (black dashed), confirming the lateral deviation pattern in \Cref{fig:lateral-deviation}. SimLingo trajectories (blue solid) follow the route centerline more closely on curves but exhibit higher-frequency oscillations compared to the smooth ACC paths. The stopping positions relative to the obstacle markers (red squares) are also visible: SimLingo stops before the obstacle in Variants~1--4, while in Variant~5 the blue trajectory extends through the obstacle location.
...
\begin{figure}[H]
  ...
  \caption{Ego-frame waypoint predictions for Variant~4 (straight section obstacle). The model decelerates from 2.8 to 0.9~m/s but makes bumper contact before fully stopping. Same visual encoding as \Cref{fig:waypoints-var1}.}
