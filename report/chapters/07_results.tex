...
\Cref{fig:ade-curve} shows the Average Displacement Error (ADE) between the model's predicted waypoints and the expert's ground-truth trajectories across all 15~epoch checkpoints, evaluated on the validation split. ADE drops from $0.447$ at epoch~0 to $0.120$ at epoch~14---a 73\% reduction. The steepest improvement occurs in the first 5~epochs; diminishing returns are visible after epoch~10, where ADE stabilizes around $0.12$. This final value means the model's predicted trajectories average approximately 12~cm from the expert's actual path, which is well within the QCar~2's lateral footprint.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{policy_vs_expert_curve.png}
  \caption{Average Displacement Error (ADE) across training epochs. The learned policy (blue) converges from 0.447 to 0.120 over 15~epochs. The expert baseline (green dashed) represents perfect imitation (ADE${=}0$).}
  \label{fig:ade-curve}
\end{figure}
